...
#+ global vars.
genename                = 'Cre05.g238687.v5.5'
primerlist              = [['ATGGCAGCCAAGGGAA','TCAGCCCCACGTGAG']]
bamlist                 = ['/ref/analysis/Cre/braker/braker.try5_mario/intron3000.merge.sorted.bam','/ref/analysis/Cre/tophat/stranded_nitrogen_sulfur/unmapped_all/polya.reads.merged.fq.sorted.bam']
...
def draw_gene(start_height,left,right,genename):
    ...
    print(text%(mx1,genestart+each_height*4,10,'%s (%s)'%(genename,strand)),file=Outfile)
...
for bam in bamlist:
   endp = draw_alignment(endp,chromosome,left,right,bam,50)
